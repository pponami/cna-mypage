...
public class MyPageViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenSehipped_then_UPDATE_1(@Payload Sehipped sehipped) {
        try {
            if (sehipped.isMe()) {
                // view 객체 조회
                List<MyPage> myPageList = myPageRepository.findByOrderId(sehipped.getOrderId());
                for(MyPage myPage : myPageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    myPage.setDeliveryId(sehipped.getId());
                    myPage.setStatus(sehipped.getStatus());
                    // view 레파지 토리에 save
                    myPageRepository.save(myPage);
                }
            }
